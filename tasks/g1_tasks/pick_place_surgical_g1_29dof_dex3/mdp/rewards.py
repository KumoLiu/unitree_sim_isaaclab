...
def get_trocar_tip_position(
    ...
    if not hasattr(env, cache_key):
        ...
        if asset_cfg.name == "trocar_1":
            tip_path = f"/World/envs/env_0/trocar_1/Trocar002/White_pos"
            root_path = f"/World/envs/env_0/trocar_1"
        elif asset_cfg.name == "trocar_2":
            tip_path = f"/World/envs/env_0/trocar_2/DisposableLaparoscopicPunctureDevice001/Red_pos"
            root_path = f"/World/envs/env_0/trocar_2"
        else:
        ...
        if not tip_prim.IsValid():
            print(f"⚠️ Warning: Tip prim not found at {tip_path}, using zero offset")
...
def trocar_tip_alignment_reward(
    ...
    Args:
        ...
        use_sparse_reward: If True, only give reward (1.0) when stage >= 2.
                          If False (default), give continuous reward based on tip distance.
    ...
    if use_sparse_reward:
        ...
        reward = torch.where(stage < 1, torch.zeros(env.num_envs, device=env.device),
                            torch.where(stage == 1, tip_reward,
                                       env._tip_reward_locked))
    ...
    if should_print_debug(env, print_log=print_log) and stage[0].item() == 1:
        ...
        print(f'   └─ Stage 1 (Find Hole, {mode_str}): tip_pos_1=({tip_pos1[0,0]:.3f}, {tip_pos1[0,1]:.3f}, {tip_pos1[0,2]:.3f}) | '
              ...
              f'tip_d={tip_dist[0].item():.4f} | reward={reward[0].item():.3f}')
...
def trocar_insertion_reward(
    ...
    if use_sparse_reward:
        ...
        reward = torch.where(stage < 2, torch.zeros(env.num_envs, device=env.device),
                            torch.where(stage == 2, insertion_reward,
                                       env._insertion_reward_locked))
...
def trocar_placement_reward(
    ...
    if use_sparse_reward:
        ...
        reward = torch.where(stage < 3, torch.zeros(env.num_envs, device=env.device),
                            torch.where(stage == 3, placement_reward,
                                       env._placement_reward_locked))
